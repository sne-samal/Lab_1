####################################################################
##
##  Physical synthesis: RTL to a finished layout in one session.
##
##  Author:   Sne Samal
##  Version:  1.0
##  Date:     2026-08-23
##
##      fc_shell -f scripts/fusion.tcl
##
##  Synthesis sees the floorplan and places cells as it maps them, so
##  it optimises against estimated real wire delays instead of none.
##  There is no netlist handoff: the placement carries straight
##  through to the clock tree and the router.
##
##  compile_fusion runs seven stages in order: initial_map, logic_opto,
##  initial_place, initial_drc, initial_opto, final_place, final_opto.
##  It is split around the floorplan below, because a core sized by
##  utilization needs a real cell area to size itself against.
##
####################################################################

set FLOW fusion
source scripts/setup.tcl

lab_banner "Physical synthesis: $DESIGN"

set_host_options -max_cores $MAX_CORES

if { [file exists $PNR_LIB] } { file delete -force $PNR_LIB }

create_lib $PNR_LIB -technology $TECH_FILE -ref_libs $REF_LIBS

report_ref_libs

####################################################################
## Read the design
####################################################################
# No set_non_physical_mode here, which is the whole difference from
# scripts/syn_logical.tcl.

analyze -format sverilog $RTL_FILES
elaborate $DESIGN
set_top_module $DESIGN

####################################################################
## Technology fix-ups
####################################################################
# The tech file gives every layer an "unknown" preferred direction and
# the site no symmetry, so the tool warns per layer and then guesses.
# Its guesses are right; setting them keeps the log readable.

suppress_message ATTR-12
set_attribute [get_layers $HORIZONTAL_LAYERS] routing_direction horizontal
set_attribute [get_layers $VERTICAL_LAYERS]   routing_direction vertical
set_attribute [get_site_defs $SITE_NAME] symmetry "$SITE_SYMMETRY"
unsuppress_message ATTR-12

set PHYSICAL 1
source scripts/mcmm.tcl

report_clocks

####################################################################
## Map, then floorplan
####################################################################
# Floorplanning an unmapped design gives a core of zero size: the tool
# says so (DPI-804) and placement then fails with PLACE-006 over
# utilization. Stopping at logic_opto gives a real cell area first, so
# the floorplan comes out the same as the two-step flow's.

lab_banner "compile_fusion -to logic_opto"

compile_fusion -to logic_opto

lab_reports synth 0
lab_headline

source scripts/floorplan.tcl

####################################################################
## Place and optimise
####################################################################
# Resumed against the floorplan just built: initial_place through
# initial_drc, initial_opto, final_place and final_opto.

lab_banner "compile_fusion -from initial_place"

compile_fusion -from initial_place -to final_opto

# Logic needing a constant 1 or 0 cannot connect straight to the rail:
# the gate oxide it drives would see the supply directly. A tie cell
# provides the constant through a transistor instead.
add_tie_cells \
    -tie_high_lib_cells [get_lib_cells */$TIE_HI_CELL] \
    -tie_low_lib_cells  [get_lib_cells */$TIE_LO_CELL]

connect_pg_net -automatic

legalize_placement

redirect -tee -file $RPT_DIR/place_legality.rpt {check_legality}

# Reported, not gated. They are gated at finish.
redirect -tee -file $RPT_DIR/place_pg_drc.rpt          {check_pg_drc}
redirect -tee -file $RPT_DIR/place_pg_connectivity.rpt {check_pg_connectivity}

lab_reports place
lab_headline
save_block -label place
save_lib

source scripts/backend.tcl
